...
def apply_update(settings: KdxSettings, *, rollback_ref: str = "") -> dict[str, Any]:
    ...
    stashed = _git_stash_push(settings)
    try:
        _run(settings, ["git", "-C", repo, "fetch", "--tags", "--prune", "origin"])
        if rollback_ref.strip():
            _run(settings, ["git", "-C", repo, "checkout", rollback_ref.strip()])
        else:
            target = _resolve_update_target(settings)
            _run(settings, ["git", "-C", repo, "reset", "--hard", target])
        _run(settings, [sys.executable, str(settings.repo_root / "bootstrap.py"), "--setup-only"])
    except Exception:
        if stashed:
            _git_stash_pop(settings)
        raise
    return {
        "repo_root": repo,
...
def _resolve_update_target(settings: KdxSettings) -> str:
    branch = _default_branch(settings)
    return f"origin/{branch}"


def _default_branch(settings: KdxSettings) -> str:
    output = _git_output(settings, ["remote", "show", "origin"])
    for line in output.splitlines():
        ...
        if stripped.startswith("HEAD branch:"):
            ...
            if candidate and candidate != "(unknown)":
                return candidate
    output = _git_output(settings, ["symbolic-ref", "refs/remotes/origin/HEAD"])
    ref = output.strip()
...
def _git_stash_push(settings: KdxSettings) -> bool:
    result = subprocess.run(
        ["git", "-C", str(settings.repo_root), "stash", "push", "--include-untracked", "-m", "kdx-auto-update"],
        capture_output=True, text=True, check=False,
    )
    return result.returncode == 0 and "No local changes" not in (result.stdout or "")


def _git_stash_pop(settings: KdxSettings) -> None:
    subprocess.run(
        ["git", "-C", str(settings.repo_root), "stash", "pop"],
        capture_output=True, text=True, check=False,
    )

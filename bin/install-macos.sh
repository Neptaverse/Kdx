...
pick_python() {
  ...
  candidates+=("/opt/homebrew/bin/python3.12" "/usr/local/bin/python3.12" "python3.12" "python3.11" "python3")
  local py
...
print_path_hint() {
  ...
  log "add this to ~/.zprofile and reopen terminal:"
...
main() {
  ...
  if ! py="$(pick_python)"; then
    fail "no stable python found (need Python 3.11 or 3.12 final). Install python@3.12 via Homebrew and retry."
  fi
